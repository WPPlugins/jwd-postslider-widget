	...
	jQuery.fn.jwdspSliderUI = function() {
		...
		switch(unit){
			...
			jwdspSlider.slider({
				...
				create: function() { handle.text( jQuery( this ).slider( 'value' )*100 + '%'); },
				slide: function( event, ui ) { handle.text( ui.value*100 + '%'); input.val( ui.value ); }
			});
			break;
			...
			jwdspSlider.slider({
				...
				slide: function( event, ui ) { handle.text( ui.value + 'px'); input.val( ui.value ); }
			});
			break;
			case 'sec': /* Seconds: range 0 to 10 */
			jwdspSlider.slider({
				min: 0, max: 10000, step: 1000, value: input.val(),
				create: function() { handle.text( jQuery( this ).slider( 'value' )/1000 + 's'); },
				slide: function( event, ui ) { handle.text( ui.value/1000 + 's'); input.val( ui.value ); }
			});
			break;
