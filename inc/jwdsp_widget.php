...
if ( !class_exists('JWDSP_PostSlider')) {
	...
	/** Construct */
		function __construct() {
			parent::__construct( 'JWDSP_PostSlider', __('JWD PostSlider', 'jwdsp'), array( 
	...
	/** Widget */
		function widget($args, $instance) { 
			...
			/**/
			$exclude_nothumb = $extra_args = $nothb_x_args = array();
			...
			if($jwdsp_post_type == 'post'){
				...
				if($jwdsp_tax_term){
					$extra_args['tax_query'] = array( 
						array( 'taxonomy' => $jwdsp_taxonomy, 'field' => 'slug', 'terms' => $jwdsp_tax_term ), 
					);
				} 
			}
			...
			if ( $listings && $listings->have_posts()) { 
				...
				<?php } 	
					echo jwdsp_slider_script( array( 'slider_id' => $this->id, 'slider_controls' => $jwdsp_controls, 'slider_pager' => $jwdsp_pager, 'slider_loop' => $jwdsp_inloop ));	
	...
	/** Form */
		function form($instance) {
			...
			$posttype_args = array(
				...
				'after'			=> sprintf( __( 'Go to %s to define more custom post types.', 'jwdsp' ),'<a href="'. esc_url( get_admin_url(null, 'admin.php?page=jwdsp_postslider_page' ) ) .'">'.__('General Settings','jwdsp').'</a>')
			);
			$taxonomy_args = array(
