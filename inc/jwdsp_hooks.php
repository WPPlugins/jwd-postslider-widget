...
if ( ! function_exists( 'jwdsp_posttype_selector' ) ) { 
	function jwdsp_posttype_selector($args){
		...
		if( isset($args['after']) && !empty($args['after'])){ $after = $args['after']; } else { $after = '';}
		...
		$output .= '</select>';
		if($after){	$output .='<br /><small>'.$after.'</small>';}
		$output .= '</div>' ;
		return $output;
	}
}
	...
	function jwdsp_text_field($args){
		...
		switch($type){
			case 'slider':
				...
				$output .= '<input id="'.$id.'" name="'.$name.'" '.$class.' value="'.$value.'" '.$disabled.' type="text" '.$placeholder.' /> ' . $after;
			break;
			default:
				$output .= '<label '.$label_style.' for="'.$id.'"> ' . $label . $loader . ' </label>';
				$output .= '<input id="'.$id.'" name="'.$name.'" '.$class.' '.$style.' value="'.$value.'" '.$disabled.' type="'.$type.'" '.$placeholder.' /> ' . $after;
		}
...
if ( ! function_exists( 'jwdsp_slider_script' ) ) { 
	function jwdsp_slider_script($args){
		...
		$output = '';
		if( $slider_id != ''){ 
			$output .= '<script type="text/javascript">jQuery(document).ready(function(){ var swiper'. preg_replace("/[^0-9]/","",$slider_id).' = new Swiper(".swiper-'.$slider_id.'", {';
			if( $slider_pager != ''){ $output .= 'pagination: ".'.$slider_id.'-pagination",'; }
			if( $slider_controls != ''){ $output .= 'nextButton: ".'.$slider_id.'-next", prevButton: ".'.$slider_id.'-prev",'; }
...
if ( ! function_exists( 'jwdsp_get_custom_css' ) ) {
	function jwdsp_get_custom_css(){
		...
		if($settings['pagination_color'] != ''){
			$css .= '.jwdsp_widget .swiper-pagination-progressbar{background-color:'.$settings['pagination_color'].'!important;}';
 ...
 ********************************/
 if ( ! function_exists( 'jwdsp_sync_options' ) ) {
	 function jwdsp_sync_options($options = array() ){
